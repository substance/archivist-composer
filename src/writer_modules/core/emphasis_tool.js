...
var EmphasisTool = React.createClass({
  ...
  handleSelectionChange: function(sel) {
    ...
    if (sel.isNull() || !sel.isPropertySelection()) {
      ...
      var annotations = writerCtrl.doc.annotationIndex.get(sel.getPath(), range[0], range[1], "emphasis");
  ...
  render: function() {
    var classNames = ['emphasis-tool-component', 'tool'];
    ...
    return $$("a", {
      ...
      dangerouslySetInnerHTML: {__html: '<i class="fa fa-italic"></i>'}
    });
  }
});
